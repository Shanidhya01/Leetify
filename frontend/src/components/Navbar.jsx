        ...
        <nav className='h-200px border border-gray-400 p-2 bg-[#f0f0f0] flex justify-center relative'>
            ...
            <div className='flex gap-1 justify-end items-center text-xl mt-1 '>
                <button className='bg-blue-500   border p-1 rounded-lg text-xl ml-10 text-white' onClick={handleUsernameClick} >
                    {Authuser ? (Authuser.username || Authuser.displayName || 'User') : 'SIGN-UP'}</button>
                <img src={Authuser ? (Authuser.profilePic || Authuser.photoURL) : ""} width={40} height={30} alt="" className="rounded-full" />
                <button className='bg-orange-200 border p-1 rounded-lg text-xl ml-10 text-orange-500' onClick={logout} disabled={loading}>
