...
const ProblemBar = () => {
  const { isProblemBar, setisProblemBar, data, setData, probId, setprobId } =
  ...
  const fetchData = async () => {
    try {
      ...
      const solvedCount = statusResults.filter(
        ({ isSolved }) => isSolved
      ).length;
  ...
  const fetchData_solved = async (prob) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/submit/${Authuser._id}/${prob}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div
      className="w-[380px] h-[1000px] top-0 left-0 flex flex-col gap-5 p-4 justify-start  bg-[#f0f0f0] absolute overflow-y-auto
    ...
    >
      ...
      <div className="mt-2 mb-4">
        <span>
          {solvedCount}/{data.length} problems solved
        </span>
      </div>
      {loading ? (
        <span>Loading...</span>
        ...
        data.map((problem, index) => (
          <div
            key={index}
            className="problem-row cursor-pointer border border-black bg-orange-300"
            onClick={() => setprobId(problem._id)}
          >
            {problem.problem_name}
            {solvedStatus[problem._id] && (
              <span className="text-green-600 ml-10">SOLVED</span>
            )}
          </div>
        ))
      )}
